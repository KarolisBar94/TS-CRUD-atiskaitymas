...
class Table<Type extends RowData> {
  ...
  private checkColumnsCompatability = (): void => {
    ...
    if (!columnsCompatableWithRowsData) {
      throw new Error('The number of columns in the table does not match');
    }
  };

  private initialize = (): void => {
    this.htmlElement.className = 'table table-striped orders border p-3 ';
  ...
  private renderHeadView = (): void => {
    ...
    this.thead.innerHTML = `
      <tr>
        <th colspan="${headersArray.length}" class="text-center h2 ">${title}</th>
      </tr>
